...
function throughput(nSTAs, SuccTx, length, MaxCycles, DL_time, UL_time, frames)
    ...
    for i = 1:nSTAs
        DL_throughput(i) = SuccTx(i) * frames(i) *length / (MaxCycles * (DL_time / MaxCycles));
    ...
    fprintf("\n-------- UPLINK TRANSMISSION -------- \n");
    for i = 1:nSTAs
        UL_throughput(i) = SuccTx(i) * frames(i) *length / (MaxCycles * (UL_time / MaxCycles));
