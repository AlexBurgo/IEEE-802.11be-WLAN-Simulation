...
% L_data: packet size (bits)
% BW: channel width
% MCS: data rate
% MCSbasic: basic data rate
% SS: SU spatial streams.
% Ns: Data Subcarriers

function [SU_DL_Time, SU_UL_Time, Nframes, prob_err] = wifiTransmissionTimeBasic(nSTAs, SS, P_rx, L_data, distance, opti, BW)

dprop = distance/3E8; % Propagation delay
Nframes = 256; % number of aggregated frames in A-MPDU

if opti == 1
    [DBPS, DBPSbasic, prob_err] = optimization(P_rx); % optimization algorithm
else
